...
import static sirius.stellar.serialization.msgpack.MessagePack.Code.ARRAY16;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.ARRAY32;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.BIN16;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.BIN32;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.BIN8;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.EXT16;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.EXT32;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.EXT8;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.EXT_TIMESTAMP;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.FALSE;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.FIXARRAY_PREFIX;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.FIXEXT1;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.FIXEXT16;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.FIXEXT2;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.FIXEXT4;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.FIXEXT8;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.FIXMAP_PREFIX;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.FIXSTR_PREFIX;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.FLOAT32;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.FLOAT64;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.INT16;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.INT32;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.INT64;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.INT8;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.MAP16;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.MAP32;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.NIL;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.STR16;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.STR32;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.STR8;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.TRUE;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.UINT16;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.UINT32;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.UINT64;
import static sirius.stellar.serialization.msgpack.MessagePack.Code.UINT8;

/// MessagePack serializer that converts objects into binary.
/// Use the factory methods of [MessagePack] or [MessagePack.PackerConfig] to instantiate.
///
/// This class provides the following primitive methods to write MessagePack values.
/// These primitive methods write short bytes (1 to 7 bytes) to the internal buffer at once.
/// There are also some utility methods for convenience.
///
/// Primitive methods:
///
...
///
/// To write a byte array, first you call [#packBinaryHeader] method with length of the byte array.
/// Then, you call [#writePayload(byte[],int,int)] or [#addPayload(byte[],int,int)] methods to write
/// the contents.
///
/// To write a [List], [Collection] or array, first you call the [#packArrayHeader(int)] method with
/// the number of elements. Then, you call packer methods for each element iteration.
///
/// To write a Map, first you call [#packMapHeader(int)] method with size of the map. Then, for each
/// pair, you call packer methods for key first, and then value. You will call packer methods twice
/// as many time as the size of the map.
///
/// Note that packXxxHeader methods don't validate number of elements. You must call packer methods
/// for correct number of times to produce valid MessagePack data.
///
/// When IOException is thrown, primitive methods guarantee that all data is written to the internal
/// buffer or no data is written. This is convenient behavior when you use a non-blocking output
/// channel that may not be writable immediately.
        ...
        implements Closeable, Flushable
{
     ...
     */
    protected MessageBufferOutput output;
    ...
    /**
     * Create an MessagePacker that outputs the packed data to the given {@link sirius.stellar.serialization.msgpack.buffer.MessageBufferOutput}.
     ...
     *
     * @param output MessageBufferOutput. Use {@link sirius.stellar.serialization.msgpack.buffer.OutputStreamBufferOutput}, {@link sirius.stellar.serialization.msgpack.buffer.ChannelBufferOutput} or
     * your own implementation of {@link sirius.stellar.serialization.msgpack.buffer.MessageBufferOutput} interface.
     */
    protected MessagePacker(MessageBufferOutput output, MessagePack.PackerConfig config)
    {
        this.output = requireNonNull(output, "MessageBufferOutput is null");
        ...
        this.str8FormatSupport = config.str8FormatSupport();
        this.position = 0;
            ...
            throws IOException
    {
        ...
        MessageBufferOutput newOut = requireNonNull(out, "MessageBufferOutput is null");

        // Flush before reset
        flush();
        MessageBufferOutput old = this.output;
        this.output = newOut;
    ...
    {
        if (buffer == null) {
            buffer = output.next(minimumSize);
        }
    ...
    private void prepareEncoder()
    {
        if (encoder == null) {
             ...
             *
             * sirius.stellar.serialization.msgpack.MessageStringCodingException: java.nio.charset.MalformedInputException: Input length = 1
             *     at sirius.stellar.serialization.msgpack.MessagePacker.encodeStringToBufferAt(MessagePacker.java:467) ~[msgpack-core-0.8.6.jar:na]
             *     at sirius.stellar.serialization.msgpack.MessagePacker.packString(MessagePacker.java:535) ~[msgpack-core-0.8.6.jar:na]
             *
     ...
     *
     * <p>
     * This method writes a UTF-8 string using the smallest format from the str format family by default. If {@link MessagePack.PackerConfig#str8FormatSupport(boolean)} is set to false, smallest format from the str format family excepting str8 format.
     *
     ...
     * array, even if it is shorter than {@link MessagePack.PackerConfig#bufferFlushThreshold(int)}. This is
     ...
     * given byte array, even if it is shorter than {@link MessagePack.PackerConfig#bufferFlushThreshold(int)}.
