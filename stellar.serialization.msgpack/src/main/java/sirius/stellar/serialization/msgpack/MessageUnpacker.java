...
import sirius.stellar.serialization.msgpack.MessagePack.Code;
...
import sirius.stellar.serialization.msgpack.exception.*;
...
/// Use the factory methods of [MessagePack] or [MessagePack.UnpackerConfig] to instantiate.
///
/// This class provides primitive methods to read MessagePack values, typically by inspecting the type
/// using [#getNextFormat()] and [MessageFormat#getValueType()], followed by the appropriate unpackXxx
/// method. These methods read short bytes (1 to 7 bytes) from the input stream at once.
/// Utility methods are also provided for convenience.
///
/// Primitive methods:
///
...
/// | Binary           | [#unpackBinaryHeader()]                  | `byte[]`              |
/// | String           | [#unpackRawStringHeader()]               | [String]              |
/// | Array            | [#unpackArrayHeader()]                   | Array/List            |
...
/// | Any              | [#unpackValue()]                         | [Value]              |
///
/// To read a byte array, first call [#unpackBinaryHeader()] to get the length of the array,
/// then use [#readPayload(int)] or [#readPayloadAsReference(int)] to read the contents.
///
/// To read an array, first call [#unpackArrayHeader()] to get the number of elements, then
/// call unpacker methods for each element iteratively.
///
/// To read a map, first call [#unpackMapHeader()] to get the number of key-value pairs, then
/// call unpacker methods for each key and value, resulting in twice as many calls as the number of pairs.
///
/// Note that unpackXxxHeader methods do not validate the number of elements. You must call unpacker
/// methods the correct number of times to process valid MessagePack data correctly.
///
/// To process a sequence of MessagePack values, use [#hasNext()] to check if more data is available,
/// then call [#unpackValue()] or other unpacker methods until [#hasNext()] returns false.
        ...
        implements Closeable
{
    ...
    {
        this.in = requireNonNull(in, "MessageBufferInput is null");
    ...
    public MessageBufferInput reset(MessageBufferInput in)
            throws IOException
    {
        MessageBufferInput newIn = requireNonNull(in, "MessageBufferInput is null");
     ...
     * MessageFormat instance. If false, next {@link #getNextFormat()} call will throw
	 * an MessageInsufficientBufferException.
     *
            ...
            throws IOException
    {
        ...
        switch (mf.getValueType()) {
            ...
            case EXTENSION: {
                ...
                default:
                    return ValueFactory.newExtension(extHeader.type(), readPayload(extHeader.length()));
                }
            }
            default:
        ...
        switch (mf.getValueType()) {
            ...
            case EXTENSION: {
                ...
                case EXT_TIMESTAMP:
                    ...
                    break;
                default:
                    var.setExtensionValue(extHeader.type(), readPayload(extHeader.length()));
                }
                return var;
            }
            default:
     ...
     *
     * The difference from {@link #unpackNil()} is that unpackNil throws an exception
	 * if the next byte is not nil value while this tryUnpackNil method returns false
    ...
    private String decodeStringFastPath(int length)
    {
        if (actionOnMalformedString == CodingErrorAction.REPLACE &&
                ...
                buffer.hasArray()) {
            String s = new String(buffer.array(), buffer.arrayOffset() + position, length, StandardCharsets.UTF_8);
            position += length;
            return s;
        }
        else {
    ...
    {
        if (ext.type() != EXT_TIMESTAMP) {
            throw unexpectedExtension("Timestamp", EXT_TIMESTAMP, ext.type());
        }
        switch (ext.length()) {
            case 4: {
