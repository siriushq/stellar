...
import sirius.stellar.serialization.msgpack.value.MapValue;
...
/**
 * {@code DImmutableMapValue} Implements {@code ImmutableMapValue} using a {@code Value[]} field.
 *
 * @see sirius.stellar.serialization.msgpack.value.MapValue
 */
public class DImmutableMapValue
        extends DAbstractImmutableValue
        implements ImmutableMapValue
{
    private static final DImmutableMapValue EMPTY = new DImmutableMapValue(new Value[0]);
    ...
    public DImmutableMapValue(Value[] kvs)
    {
        this.kvs = kvs;
    }
    ...
    private static void appendJsonKey(StringBuilder sb, Value key)
    {
        if (key.isRawValue()) {
            ...
            DImmutableStringValue.appendJsonString(sb, key.toString());
        }
    }
